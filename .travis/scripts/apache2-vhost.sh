...
sed s?%basedir%?$ROOTDIR? "$BASEDIR/apache2/virtualhost.local-dist" | sed s/%hostname%/$VHOSTNAME/ > virtualhost.local
sudo mv virtualhost.local /etc/apache2/sites-available/virtualhost.local
